...
function RevenueReport() {
  ...
  const formatChartData = (chartData) => {
        ...
        chartData[datasetKey].forEach((entry) => {
          ...
          if (!monthlyTotals[formattedDate]) {
            monthlyTotals[formattedDate] = {
              ...
              ReferralLinks: 0,
              Refunds: 0,
              tooltipDate: dateObj.toISOString().split("T")[0],
            };
          }

          monthlyTotals[formattedDate][key] += amount;
          monthlyTotals[formattedDate][key] = parseFloat(
            monthlyTotals[formattedDate][key].toFixed(2)
          );

          if (
            dateObj.toISOString().split("T")[0] >
            monthlyTotals[formattedDate].tooltipDate
          ) {
            monthlyTotals[formattedDate].tooltipDate =
              dateObj.toISOString().split("T")[0];
          }
    ...
    while (cursor <= end) {
      ...
      if (!monthlyTotals[formatted]) {
        monthlyTotals[formatted] = {
          ...
          ReferralLinks: 0,
          Refunds: 0,
          tooltipDate: cursor.toISOString().split("T")[0],
        };
      }
    ...
    return Object.values(monthlyTotals).sort((a, b) => {
      const d1 = new Date(a.tooltipDate);
      const d2 = new Date(b.tooltipDate);
      return d1 - d2;
    });
  };
  ...
  const filterData = (data, filter) => {
    ...
    switch (filter) {
      ...
      case "5 Years":
        return data.filter((item) => {
          const itemDate = parseMonthYear(item.name);
          return itemDate >= getDateOffset(60);
        });
      ...
      <Card className="p-2">
        <Card className="my-3">
          <ResponsiveContainer width="100%" height={400}>
            <AreaChart
              data={filterData(formattedChartData, filter)}
